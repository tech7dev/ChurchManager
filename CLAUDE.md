...
| 15 | **Blazor Admin** | ✅ COMPLETE | MudBlazor 9.x UI: JWT auth (JwtAuthenticationStateProvider + ProtectedLocalStorage), AuthorizeRouteView + RedirectToLogin, named HttpClient services per module (MemberService, EventService, FinanceService, ReportService, DepartmentService, MessagingService, SubscriptionService, ITManagementService), MudLayout+AppBar+Drawer shell, full NavMenu (all 21 modules), Login page, Dashboard (KPI cards), MemberList+MemberDetail, EventList, ContributionList, ExpenseList, FinancialReport, AttendanceReport, MemberReport pages. BlazorAdmin → Application project reference for DTO reuse. **Role-based authorization enforced:** 12 policies mirror the API in `Program.cs` (`RequireSuperAdmin`, `RequireChurchAdmin`, `RequireTreasurer`, etc.); every page carries `[Authorize(Roles = "...")]`; `NavMenu` links wrapped in `<AuthorizeView Roles="...">`. **Critical fix:** `JwtAuthenticationStateProvider` must pass `roleType: "role"` to the `ClaimsIdentity` constructor — `JwtSecurityTokenHandler.OutboundClaimTypeMap` rewrites `ClaimTypes.Role` to the short name `"role"` in the JWT payload, so the default role claim type URI does not match and `IsInRole(...)` / `[Authorize(Roles=...)]` silently admit everyone. |
...
- **Seed data (DRC / French):** [scripts/seed-data.sql](scripts/seed-data.sql) populates every table with République Démocratique du Congo demo data (3 churches: Kinshasa parent + Lubumbashi + Goma antennes, CDF/USD currencies, +243 phone prefix, SNEL/REGIDESO/Rawbank/M-Pesa vendors). Idempotent via parent-church existence check. Run with `sqlcmd -S ... -d ... -U ... -P ... -C -f 65001 -i scripts/seed-data.sql`.
- **Demo users:** [DemoUserSeedData.cs](src/ChurchMS.Persistence/Seed/DemoUserSeedData.cs) seeds 32 users (multiple per role) at API startup; [SuperAdminSeedData.cs](src/ChurchMS.Persistence/Seed/SuperAdminSeedData.cs) seeds `admin@churchms.com`. **All demo/superadmin passwords are `Password@123`** (single dev password).
